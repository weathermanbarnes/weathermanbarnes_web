...
def replace_values_above_equator(da1: xr.DataArray, da2: xr.DataArray, lat_name: str = 'latitude') -> xr.DataArray:
    """
...
def get_q_pte(datafile):
    ...
    data['pte850']=pte850.metpy.dequantify()
...
def get_IPV_data(datafile,data_dt):
    lats=datafile['latitude']
    lons=datafile['longitude']
    levs=datafile['isobaricInhPa']
    u=datafile['u']
    ...
    dtPT_SH=potential_temperature(data_dt.isel(potentialVorticity=1).pres, data_dt.isel(potentialVorticity=1).t)
    dtPT_NH=potential_temperature(data_dt.isel(potentialVorticity=0).pres, data_dt.isel(potentialVorticity=0).t)
    dtPT=replace_values_above_equator(dtPT_SH,dtPT_NH, lat_name='latitude')
    #dtPT=interpolate_to_isosurface(np.abs(pv.metpy.dequantify().values) * units(pv.metpy.dequantify().units),
    #                           pt.metpy.dequantify().values * units(pt.metpy.dequantify().units),
    #                           2*1e-6 * units(pv.metpy.dequantify().units))
    #dtPT=(pt.isel(isobaricInhPa=0).metpy.dequantify()*0+dtPT).drop_vars('isobaricInhPa').metpy.dequantify()
    #
    ...
    data['dtPT']=dtPT
    data['dtU']=replace_values_above_equator(data_dt.isel(potentialVorticity=1).u,
                                             data_dt.isel(potentialVorticity=0).u, lat_name='latitude')
    data['dtV']=replace_values_above_equator(data_dt.isel(potentialVorticity=1).v,
                                             data_dt.isel(potentialVorticity=0).v, lat_name='latitude')
...
def get_upper_data(datafile,msl,pwat):
    mslp=msl.prmsl/100
    pwat=pwat.pwat
