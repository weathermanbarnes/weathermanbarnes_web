...
for t in range(timestep,timestep+24,24):
    ...
    url = 'https://data.ecmwf.int/forecasts/'+init_date+'/'+init_hour+'z/ifs/0p25/enfo/'+init_date+init_time+'-'+str(t)+'h-enfo-ef'
    p = subprocess.Popen(['wget',url+'.index'],stdout=subprocess.PIPE)
    os.waitpid(p.pid,0)
    print("done")
