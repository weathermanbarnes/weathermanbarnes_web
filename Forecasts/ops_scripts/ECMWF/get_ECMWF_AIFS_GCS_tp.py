...
for t in range(timestep,timestep+24,24):
    ...
    url = url_main+init_date+'/'+init_hour+'z/aifs-ens/0p25/enfo/'+init_date+init_time+'-'+str(t)+'h-enfo-pf'
    ...
    for ltype,llist,para in zip(ltypes,llists,paras):
        ...
        for index,row in df.iterrows():
            ...
            outfile=datadir+row.date+row.time+'00-'+row.step+'h-aifs-enfo-pf_'
